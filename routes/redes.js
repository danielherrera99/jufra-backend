...
router.post('/', proteger, autorizarRoles('admin', 'consejo'), async (req, res) => {
    try {
        const nuevaPublicacion = await RedSocialPost.create(req.body);
...
router.put('/:id', proteger, autorizarRoles('admin', 'consejo'), async (req, res) => {
    try {
        ...
        // Simular update del ORM
        await RedSocialPost.update(req.params.id, req.body);

        res.json({ success: true, data: { ...post, ...req.body } });
    } catch (error) {
        console.error(error);
...
router.delete('/:id', proteger, autorizarRoles('admin', 'consejo'), async (req, res) => {
    try {
        ...
        await RedSocialPost.delete(req.params.id);
